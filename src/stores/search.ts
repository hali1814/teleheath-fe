  ...
  persist(
    (set, get) => ({
      ...
      addRecent: (query) => {
        ...
        const filtered = current.filter((q) => q.trim().toLowerCase() !== lower)
...
export const useSpecialtySearchStore = create<SpecialtySearchStore>()(
  persist(
    (set, get) => ({
      recentSearches: [],
    ...
    {
      name: 'specialty-search-storage',
      partialize: (state) => ({
        recentSearches: state.recentSearches,
      }),
      merge: (persisted, current) => {
        const p = persisted as Partial<SpecialtySearchStore> | undefined
        const list = p?.recentSearches
        if (Array.isArray(list) && list.length > 5) {
          return {
            ...current,
